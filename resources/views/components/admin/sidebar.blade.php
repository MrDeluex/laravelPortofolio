...
<div id="overlay" class="hidden fixed inset-0 bg-gradient-to-tr from-gray-900 via-gray-800 to-gray-700 opacity-50 z-20"></div>

<!-- Sidebar (hanya satu sidebar untuk semua ukuran layar) -->
<div id="sidebar"
    class="hidden md:flex flex-col items-center w-52 min-h-screen h-full overflow-hidden text-gray-400 bg-gradient-to-bl from-gray-900 via-gray-800 to-gray-700 z-30 fixed">
    <a class="flex items-center w-full px-3 mt-3" href="{{ route('admin.dashboard') }}">
    ...
    <div class="relative w-full mt-auto">
        <a id="account-btn" class="flex items-center justify-center w-full h-16 mt-auto bg-gray-900 hover:bg-gray-700 hover:text-gray-300 cursor-pointer">
        ...
        <div id="account-dropdown" class="hidden absolute bottom-16 left-0 w-full bg-gray-900 text-white">
            {{-- <a href="/profile" class="block px-4 py-2 hover:bg-gray-700">Profile</a> --}}
            <form method="POST" action="{{ route('logout') }}">
                @csrf
                <button type="submit" class="block w-full px-4 py-2 bg-gray-700 hover:bg-gray-600" role="menuitem" tabindex="-1" id="user-menu-item-2">
                    {{ __('Log Out') }}
                </button>
            </form>
        </div>
